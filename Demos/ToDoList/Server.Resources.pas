...
uses
    StrUtils
  , Yichain.Core.Registry, Yichain.Rtti.Utils
...
begin
  ...
  if LCommand.RowsAffected < 1 then
    raise EYichainHttpException.CreateFmt('Item %d not found or access denied', [id], 404);
end;
...
begin
  ...
  LQuery.Open;
  if LQuery.IsEmpty then
    raise EYichainHttpException.CreateFmt('Item %d not found or access denied', [AItem.Id], 404);
  ...
  FD.Query('select * from ACCOUNT where ID = :Token_Claim_ACCOUNT_ID', nil, nil,
    ...
    begin
      if AQuery.IsEmpty then
        raise EYichainHttpException.Create('Account not found or access denied', 404);
...
initialization
  TYichainResourceRegistry.Instance.RegisterResource<TItemResource>;
  TYichainResourceRegistry.Instance.RegisterResource<TAccountResource>;
