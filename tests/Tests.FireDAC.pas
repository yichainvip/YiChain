...
, Yichain.Core.MessageBodyReader, Yichain.Core.MessageBodyWriter
, Yichain.Core.MessageBodyReaders, Yichain.Core.MessageBodyWriters
, Yichain.Data.FireDAC
, Yichain.Data.FireDAC.ReadersAndWriters
, Yichain.Data.FireDAC.Utils
, Yichain.Core.JSON
;

type
  [TestFixture('MBW_FireDAC')]
  TYichainFireDACReaderWriterTest = class(TObject)
  private
...
{ TYichainFireDACWriterTest }

procedure TYichainFireDACReaderWriterTest.ArrayOfDataSets;
var
...
procedure TYichainFireDACReaderWriterTest.ArrayOfDataSetsInTValue;
var
  LDataSet: TFDMemTable;
  LDataSets: TArray<TFDMemTable>;
  LName: string;
...
procedure TYichainFireDACReaderWriterTest.ParamsInjection;
var
  LFD: TYichainFireDAC;
  LQuery: TFDQuery;
begin
  LFD := TYichainFireDAC.Create('MAIN_DB');
  try
    LQuery := LFD.CreateQuery('select * from EMPLOYEE where EMP_NO = :Custom_VALUE1', nil, False);
    try
      LFD.AddContextValueProvider(
        procedure (const AActivation: IYichainActivation;
...
procedure TYichainFireDACReaderWriterTest.Setup;
begin
...
procedure TYichainFireDACReaderWriterTest.TearDown;
begin
  FJSONMediaType.Free;
  FOutputStream.Free;
end;
...
initialization
  TDUnitX.RegisterTestFixture(TYichainFireDACReaderWriterTest);
